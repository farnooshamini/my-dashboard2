...
const fail = (res, message, status)    => res.status(status).json({ success: false, message });

const isAdmin = (req) => req.user?.role === 'administrator';

async function getAll(req, res) {
    try {
        const where = isAdmin(req) ? {} : { assignedTo: req.user.id };
...
async function create(req, res) {
    ...
    const { message, status, priority, channel } = req.body;
    const assignedTo = isAdmin(req) ? (req.body.assignedTo ?? null) : req.user.id;
